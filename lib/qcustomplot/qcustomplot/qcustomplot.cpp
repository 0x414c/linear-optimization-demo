...
{
  mColorBuffer.fill(qRgba(0, 0, 0, 255), mLevelCount);
  loadPreset(preset);
}
...
void QCPColorGradient::updateColorBuffer()
{
      ...
      } else // position is in between stops (or on an intermediate stop), interpolate color
      {
        ...
        switch (mColorInterpolation)
        {
          ...
          case ciHSV:
          {
            ...
            mColorBuffer[i] = QColor::fromHsvF(hue, (1-t)*lowHsv.saturationF() + t*highHsv.saturationF(), (1-t)*lowHsv.valueF() + t*highHsv.valueF(), (1-t)*lowHsv.alphaF() + t*highHsv.alphaF()).rgba();
            break;
          }
        }
      }
  ...
  {
    mColorBuffer.fill(mColorStops.constBegin().value().rgba());
  } else // mColorStops is empty, fill color buffer with black
  {
    mColorBuffer.fill(qRgba(0, 0, 0, 255));
  }
...
void QCPColorScaleAxisRectPrivate::updateGradientImage()
{
  ...
  if (mParentColorScale->mType == QCPAxis::atBottom || mParentColorScale->mType == QCPAxis::atTop)
  {
    w = n;
    h = rect().height();
    mGradientImage = QImage(w, h, QImage::Format_ARGB32);
    QVector<QRgb*> pixels;
    for (int y=0; y<h; ++y)
...
void QCPColorMap::updateMapImage()
{
  ...
  if (keyAxis->orientation() == Qt::Horizontal && (mMapImage.width() != keySize*keyOversamplingFactor || mMapImage.height() != valueSize*valueOversamplingFactor))
    mMapImage = QImage(QSize(keySize*keyOversamplingFactor, valueSize*valueOversamplingFactor), QImage::Format_ARGB32);
    ...
    mMapImage = QImage(QSize(valueSize*valueOversamplingFactor, keySize*keyOversamplingFactor), QImage::Format_ARGB32);
  ...
  if (keyOversamplingFactor > 1 || valueOversamplingFactor > 1)
  {
    ...
    if (keyAxis->orientation() == Qt::Horizontal && (mUndersampledMapImage.width() != keySize || mUndersampledMapImage.height() != valueSize))
      mUndersampledMapImage = QImage(QSize(keySize, valueSize), QImage::Format_ARGB32);
    else if (keyAxis->orientation() == Qt::Vertical && (mUndersampledMapImage.width() != valueSize || mUndersampledMapImage.height() != keySize))
      mUndersampledMapImage = QImage(QSize(valueSize, keySize), QImage::Format_ARGB32);
