...
GraphicalSolver2D::GraphicalSolver2D(const LinearProgramData<real_t>& linearProgramData) :
  _linearProgramData(linearProgramData) {}

/**
 * @brief GraphicalSolver2D::solve
 * Naive algorithm for solving linear optimization problems
 * @return
 */
optional<PlotData2D> GraphicalSolver2D::solve()
{
  //input data
  const int rows = _linearProgramData.constraintsCoeffs.rows();
  const int cols = _linearProgramData.constraintsCoeffs.cols();

  //for further computations
  LinearFunction<real_t> objFunc(_linearProgramData.objFuncCoeffs);

  //for output
  PlotData2D plotData2D(
    ResultType::Unknown,
    QPointF(numeric_limits<double>::max(), numeric_limits<double>::max()),
    numeric_limits<double>::max(),
    QVector<QPointF>(0),
    QLineF(
      QPointF(0, 0),
      QPointF(objFunc.coeffAt(0), objFunc.coeffAt(1))
    )
  );
  ...
  Matrix<real_t, Dynamic, Dynamic> augm(rows, cols + 1);
  augm <<
    _linearProgramData.constraintsCoeffs,
    _linearProgramData.constraintsRHS;

  //for the system `A * x == b' to be consistent
  //rank(A) should be equal to rank(A|b)
  FullPivLU<Matrix<real_t, Dynamic, Dynamic>> augmLU(augm);
  int augmRank = augmLU.rank();
  FullPivLU<Matrix<real_t, Dynamic, Dynamic>> coeffLU(_linearProgramData.constraintsCoeffs);
  int rank = coeffLU.rank();
  qDebug() << "The rank of A is " << rank;
  qDebug() << "The rank of A|b is " << augmRank;
  if (rank != augmRank || (_linearProgramData.constraintsCoeffs.array() == 0).all())
  {
  ...
  Matrix<real_t, Dynamic, Dynamic> ker = coeffLU.kernel();
  Matrix<real_t, Dynamic, Dynamic> im = coeffLU.image(_linearProgramData.constraintsCoeffs);
...
//  cerr << ck << endl;
  ...
  Matrix<real_t, Dynamic, Dynamic> coeffs(rows + 2, cols);
  coeffs <<
    _linearProgramData.constraintsCoeffs,
    Matrix<real_t, 2, 2>::Identity(2, 2) * -1;

  Matrix<real_t, Dynamic, 1> RHS(rows + 2, 1);
  RHS <<
    _linearProgramData.constraintsRHS,
    Matrix<real_t, 2, 1>::Zero(2, 1);
  ...
  for (int i = 0; i < (rows + 2) - 1; ++i)
  {
    ...
    for (int j = i + 1; j < (rows + 2); ++j)
    {
      ...
      Matrix<real_t, 2, Dynamic> eqs(2, cols);
      eqs <<
        coeffs.row(i),
        coeffs.row(j);
      //take corresponding RHS elements
      Matrix<real_t, 2, 1> rhs(2, 1);
      rhs <<
        RHS(i),
        RHS(j);
      ...
      Matrix<real_t, Dynamic, 1> sol = eqs.fullPivLu().solve(rhs);
      bool isValid = (eqs * sol).isApprox(rhs);
      real_t err = (eqs * sol - rhs).norm() / rhs.norm();
      ...
      bool isFeasible = isPointInFeasibleRegion<real_t>(sol, _linearProgramData);
      ...
      {
        plotData2D.vertices.append(QPointF(sol(0), sol(1)));
        real_t currValue = objFunc.evaluateAt(sol);
  ...
  //now we should sort all the found vertices in clockwise order
  //to plot a polygon later
